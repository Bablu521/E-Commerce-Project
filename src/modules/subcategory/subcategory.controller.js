...
export const updateSubcategory = asyncHandler(async(req,res,next)=>{
    ...
    if (!subcategory){
        return next (new Error("Subcategory Not Found" , {status:404}))
    }
    if (subcategory.createdBy.toString() != req.user._id.toString()){
        return next (new Error("You Are Not Allowed To Update Subcategory" , {status:403}))
    }
    if (req.file){
...
export const deleteSubcategory = asyncHandler(async(req,res,next)=>{
    ...
    if (subcategory.createdBy.toString() != req.user._id.toString()){
        return next (new Error("You Are Not Allowed To Delete Subcategory" , {status:403}))
    }
    await subcategory.deleteOne()
    await cloudinary.uploader.destroy(subcategory.image.id)
    return res.status(200).json({message:"Done" , deletedSubcategory})
})
